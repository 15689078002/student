...
void add()                                      //成绩的录入与添加函数
{
    ...
    system("clear");                             //清屏函数  系统中调用
    person p[200]  ;
    while(sign!='n'&&sign!='N')                    //其实只有当输入N或n时才跳出循环
    {
        printf("             ----学生纪录添加----\n");
        printf("姓名：\n");
        scanf("\t%s",p[i].name);//数组不加& 本身就是地址
        printf("学号:\n");
        scanf("\t%s",p[i].num);
        printf("C语言：\n");
        scanf("\t%f",&p[i].cyuyan);
...
void scan()                        //显示记录  进行全面信息浏览
{
   //void amend();
    ...
    int i=0;
    while(fscanf(fp,"%s%s%f%f%f%f%f",per[i].name,per[i].num,&per[i].cyuyan,&per[i].en,&per[i].ji,&per[i].guti,&per[i].cailiao)!=EOF)//表示文件未到末尾
    {
...
void query()                     //学生信息的不同方式查询
{
    ...
    if(m==1)                                                 //模糊查询
    {
        ...
        for(i=0;i<n-1;i++)                                               //文件信息已经读取到数组中 此处进行循环用于下一语句的查找
        {
            ...
            if(p[i].name[0]==s[0]&&p[i].name[1]==s[1])              //比较per[i].name与所输入的前俩个字符是否相同，一个汉子占两个字节//
            {
                printf("\t学号: %s\n",p[i].num);
                printf("\t姓名: %s\n",p[i].name);
                printf("\tC语言: %f\n",p[i].cyuyan);
...
int main()
{
    ...
    printf("请输入用户名\n");
    scanf("%s",name);
    if(strcmp(name,"fengjingxu")==0)
    {
        printf("请输入密码\n");
        {
            scanf("%s",password);
            if (strcmp(password,"996115")==0)
            {
                printf("欢迎进入学生管理系统\n") ;
            }
            else
